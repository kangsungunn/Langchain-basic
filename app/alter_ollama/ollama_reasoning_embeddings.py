"""
Reasoning 임베딩 모델 코드 생성 스크립트

ExaOne 모델을 사용하여 ReasoningTaskEmbedding SQLAlchemy ORM 모델 코드를 자동 생성.
"""
...
def read_existing_model_file() -> str:
    """기존 Reasoning 모델 파일 읽기"""
    model_file = project_root / "app" / "domain" / "v1" / "minso" / "models" / "bases" / "reasoning.py"
...
def build_prompt(existing_model_content: str) -> str:
    ...
    prompt = f"""다음 SQLAlchemy ReasoningTask 모델을 참고하여 ReasoningTaskEmbedding ORM 클래스를 작성하세요.

=== ReasoningTask 모델 코드 ===
{existing_model_content}

=== Alembic 마이그레이션 테이블 스키마 ===
테이블명: reasoning_task_embeddings
컬럼:
- id: BigInteger, PK, autoincrement=True, nullable=False, comment='임베딩 레코드 고유 식별자'
- reasoning_task_id: String(36), FK -> reasoning_tasks.id, nullable=False, ondelete='CASCADE', comment='추론 작업 ID'
...
3. Vector 사용법: Vector(768) 형식 사용 (Vector('768'), Vector['768d'], VectorType('float64', dimension=768) 등 금지)
...
6. ForeignKey: ondelete='CASCADE' 반드시 포함 (예: ForeignKey("reasoning_tasks.id", ondelete='CASCADE'))
7. relationship: reasoning_task (back_populates="embeddings") 설정 (ReasoningTask 모델에 embeddings 관계가 있다고 가정)
8. reasoning.py의 코딩 스타일과 일관성 유지 (주석 형식, Column 정의 방식 등)
9. 모든 Column에 comment 추가 (nullable=False인 경우에도 명시)
10. __tablename__ = "reasoning_task_embeddings" 사용
11. 파일 헤더 docstring: """Reasoning Embedding - 모델 (자동 생성)\n\nReasoningTask 엔티티의 임베딩 모델.\nExaOne 모델로 자동 생성됨.\n"""
12. 클래스 docstring: """ReasoningTask 임베딩 엔티티\n\n추론 작업에 대한 벡터 임베딩 정보를 저장합니다.\n"""
13. Python 코드만 출력 (주석이나 설명 없이 순수 코드만, 원본 모델 클래스(ReasoningTask, ReasoningResult 등) 포함 금지)
...
def main():
    """메인 함수"""
    print("[ExaOne] Reasoning 임베딩 모델 코드 생성 시작...")

    try:
        print("[ExaOne] 기존 Reasoning 모델 파일 읽기...")
        ...
        output_path = project_root / "app" / "domain" / "v1" / "minso" / "models" / "bases" / "reasoning_embeddings.py"
