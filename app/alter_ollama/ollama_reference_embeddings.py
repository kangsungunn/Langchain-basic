"""
Reference 임베딩 모델 코드 생성 스크립트

ExaOne 모델을 사용하여 ReferenceAnswerEmbedding SQLAlchemy ORM 모델 코드를 자동 생성.
"""
...
def read_existing_model_file() -> str:
    """기존 Reference 모델 파일 읽기"""
    model_file = project_root / "app" / "domain" / "v1" / "minso" / "models" / "bases" / "reference.py"
...
def build_prompt(existing_model_content: str) -> str:
    ...
    prompt = f"""다음 SQLAlchemy ReferenceAnswer 모델을 참고하여 ReferenceAnswerEmbedding ORM 클래스를 작성하세요.

=== ReferenceAnswer 모델 코드 ===
{existing_model_content}

=== Alembic 마이그레이션 테이블 스키마 ===
테이블명: reference_answer_embeddings
컬럼:
- id: BigInteger, PK, autoincrement=True, nullable=False, comment='임베딩 레코드 고유 식별자'
- reference_answer_id: String(36), FK -> reference_answers.id, nullable=False, ondelete='CASCADE', comment='모범답안 ID'
...
6. ForeignKey: ondelete='CASCADE' 반드시 포함 (예: ForeignKey("reference_answers.id", ondelete='CASCADE'))
7. relationship: reference_answer (back_populates="embeddings") 설정 (ReferenceAnswer 모델에 embeddings 관계가 있다고 가정)
8. reference.py의 코딩 스타일과 일관성 유지 (주석 형식, Column 정의 방식 등)
9. 모든 Column에 comment 추가 (nullable=False인 경우에도 명시)
10. __tablename__ = "reference_answer_embeddings" 사용
11. 파일 헤더 docstring: """Reference Embedding - 모델 (자동 생성)\n\nReferenceAnswer 엔티티의 임베딩 모델.\nExaOne 모델로 자동 생성됨.\n"""
12. 클래스 docstring: """ReferenceAnswer 임베딩 엔티티\n\n모범답안에 대한 벡터 임베딩 정보를 저장합니다.\n"""
...
def main():
    """메인 함수"""
    print("[ExaOne] Reference 임베딩 모델 코드 생성 시작...")

    try:
        # 1. 기존 모델 파일 읽기
        print("[ExaOne] 기존 Reference 모델 파일 읽기...")
        ...
        # 7. 파일에 저장
        output_path = project_root / "app" / "domain" / "v1" / "minso" / "models" / "bases" / "reference_embeddings.py"
